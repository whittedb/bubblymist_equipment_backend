...
from db.models.machines import Washer, Dryer, MachineType
...
def test_disable_enable_machine(client):
    with client as c:
        ...
        for clazz in [Washer, Dryer]:
            # Get and disable machine
            machine = helpers.enable_machine(client, clazz, number=1, enable=False)
            ...
            machine = helpers.enable_machine(client, clazz, machine_id=machine_id)
...
def test_disable_create_get_machine_with_same_machine_number(client):
    with client as c:
        ...
        dryer = helpers.enable_machine(c, Dryer, number=2, enable=False)
        ...
        dryer = helpers.enable_machine(c, Dryer, machine_id=disabled_dryer_id)
        assert dryer["active"]
